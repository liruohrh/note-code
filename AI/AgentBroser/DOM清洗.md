...
- 可能渲染比较慢，导致文本未出现，应该等2次
- 这个未渲染可能会与仅仅只是未出现在snapshot里冲突，像“其他登录选项”（icon）
- 解决方法：应该等2次，2次都等1.5s

# 索引与太动态的元素变化
- 动态元素索引变化比较频繁，会导致索引一会1一会10的
- 但是这种情况相对比较少，一般情况下，这都是动画元素，不会出现在snapshot里。
...
- 要注意icon语义化（通常都是类名中包含icon信息，即语义化信息）。如果有很多同级元素如i、span、图片，然后前面一点或者后面一点有提到“某些东西”如“其他登录选项”（通常只是文本提示，不可交互），那么这些元素大概率就是这些item，此时可以尝试获取父级html，分析html（非文本匹配，而是className、id匹配）。即便没有很多相同元素，如果提到“某些东西”且在其附近文本中没有对应信息，则应该尝试分析父级html来找到这些元素，而不是说找某些元素去点击，比如“其他登录选项”（这很明显不是在文本后面，就是文本前面，通常都是在后面）。
- 结果
	- 都可以发现图标登录选项
		- agent-browser不行
